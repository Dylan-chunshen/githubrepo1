...
var t=$(_15d);
return {hasNullOpt:t.attr("hasNullOpt"),nullValue:t.attr("nullValue"),nullText:t.attr("nullText"),url:t.attr("url"),method:(t.attr("method")?t.attr("method"):undefined),checkbox:(t.attr("checkbox")?t.attr("checkbox")=="true":undefined),cascadeCheck:(t.attr("cascadeCheck")?t.attr("cascadeCheck")=="true":undefined),onlyLeafCheck:(t.attr("onlyLeafCheck")?t.attr("onlyLeafCheck")=="true":undefined),animate:(t.attr("animate")?t.attr("animate")=="true":undefined),lines:(t.attr("lines")?t.attr("lines")=="true":undefined),dnd:(t.attr("dnd")?t.attr("dnd")=="true":undefined)};
};
$.fn.tree.defaults={isFirstUrlLoad:true,hasNullOpt:false,nullValue:"",nullText:"--请选择--",url:null,method:"post",animate:false,checkbox:false,cascadeCheck:true,onlyLeafCheck:false,lines:false,dnd:false,data:null,loader:function(_15e,_15f,_160){
var opts=$(this).tree("options");
if(!opts.url){
return false;
}
$.ajax({type:opts.method,url:opts.url,data:_15e,dataType:"json",success:function(data){
if(opts.hasNullOpt&&opts.isFirstUrlLoad)
	data.unshift({'id':opts.nullValue,'text':opts.nullText,'state':'open'});
opts.isFirstUrlLoad=false;
_15f(data);
},error:function(){
...
var _6d0=$("<input type=\"hidden\" name=\""+name+"\" class=\"combo-value\">").appendTo(_6cf);
if(name){
_6d0.attr("name",name);
}
_6d0.val(_6cd[i]);
...
//edit by taohui 20130702 设置值为空，仅添加hidden
if(_6cd.length==0)
	$("<input type=\"hidden\" name=\""+name+"\" class=\"combo-value\">").appendTo(_6cf);
...
var t=$(_6e0);
return $.extend({},$.fn.validatebox.parseOptions(_6e0),{width:(parseInt(_6e0.style.width)||undefined),panelWidth:(parseInt(t.attr("panelWidth"))||undefined),panelHeight:(t.attr("panelHeight")=="auto"?"auto":parseInt(t.attr("panelHeight"))||undefined),separator:(t.attr("separator")||undefined),multiple:(t.attr("multiple")?(t.attr("multiple")=="true"||t.attr("multiple")==true||t.attr("multiple")=="multiple"):undefined),editable:(t.attr("editable")?t.attr("editable")=="true":undefined),disabled:(t.attr("disabled")?true:undefined),hasDownArrow:(t.attr("hasDownArrow")?t.attr("hasDownArrow")=="true":undefined),value:(t.val()||undefined),nullValue:t.attr("nullValue"),nullText:t.attr("nullText"),delay:(t.attr("delay")?parseInt(t.attr("delay")):undefined)});
};
$.fn.combo.defaults=$.extend({},$.fn.validatebox.defaults,{width:"auto",panelWidth:null,panelHeight:200,multiple:false,separator:",",editable:true,disabled:false,hasDownArrow:true,value:"",delay:200,nullValue:"",nullText:"--请选择--",keyHandler:{up:function(){
},down:function(){
},enter:function(){
},query:function(q){
...
for(var i=0;i<data.length;i++){
//edit by taohui 20130427处理选择到空值
if(data[i][opts.valueField]==_6f3||''==_6f3){
opts.onSelect.call(_6f2,''==_6f3?null:data[i]);
if(''==_6f3){
	$(_6f2).next().find("input.combo-value").val("");
	$(_6f2).next().find("input.combo-text").val();
}
return;
}
}
...
_703.empty();
var nullItem=$("<div class=\"combobox-item\"></div>").appendTo(_703);
