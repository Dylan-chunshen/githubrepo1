$.extend({
    UUID:{
    	create:function(){
    		var ret = "";
    		var rnd = this.Kybos();
			for (; ret.length < 32; ret += Math.floor(rnd() * 0xffffffff)
					.toString(16))
				;
			return [ ret.substr(0, 8), ret.substr(8, 4),
					"4" + ret.substr(12, 3),
		...
		sequential : function() {
			...
			for (; ret.length < 12; ret = "0" + ret)
				;
			ret = ret.substr(ret.length - 12, 12); // only least significant
													// part
			for (; ret.length < 32; ret += Math.floor(rnd() * 0xffffffff)
					.toString(16))
				;
    	...
    	Kybos:function(){
    		...
    		return (function(args) {
    			...
    			random.fract53 = function() {
    				return random() + (random() * 0x200000 | 0)
...
(function() {
	...
	var UUID = {
		...
		"parse" : function(input) {
			var ret = input.toString().trim().toLowerCase().replace(
					/^[\s\r\n]+|[\{\}]|[\s\r\n]+$/g, "");
			if ((/[a-f0-9]{8}\-[a-f0-9]{4}\-[a-f0-9]{4}\-[a-f0-9]{4}\-[a-f0-9]{12}/)
					.test(ret))
				return ret;
			else
