...
//校验Form下的所有元素
function validForm2(formElement,force){
...
function validForm(formElement,force){
	var boxes=$(".validatebox-text",$(formElement));
	for (var i=0;i<boxes.length;i++){
		if (!validValue(boxes[i],true)) return false;
	}
	//验证easyui-combotree不能为空 add by taohui 2012-11-27
	var combotrees=$(".easyui-combotree",$(formElement));
	for (var i=0;i<combotrees.length;i++){
		if (!validEnableBlank(combotrees[i],force)) return false;
	}
	return true;
}
...
//联动相关
function  getLinkageValue(thisObj,linkageName){
	var thisObjText = thisObj.value;//得到一级的显示值
    var thisObjValue = thisObj.parentElement.firstChild.value;//得到一级的实际值
	var linkageObj = document.all(linkageName);//二级显示框
	var linkageTempObj = linkageObj.parentElement.firstChild;//二级的实际值存放Text框
	var linkageSelectObj = linkageObj.parentElement.lastChild.firstChild;//二级下拉框
	var hiddenValueObj = linkageObj.parentElement.parentElement.lastChild;//二级数据存储下拉框

	//alert("linkageTempObj="+linkageTempObj)
	//alert("linkageTempObj.tagName="+linkageTempObj.tagName)

	//清空联动下拉中的数据
	linkageObj.value= "";
	linkageTempObj.value = "";
	linkageSelectObj.options.length = 0;
	
	//判断是否是清空选择
	if(thisObjText == "") thisObjValue = "";


	//alert("thisObjText="+thisObjText)
	//alert("thisObjValue="+thisObjValue)
	//alert("hiddenValueObj="+hiddenValueObj.options.length)
	

if(thisObjValue !=null && thisObjValue!=""){
	for(var i=0;i<hiddenValueObj.options.length;i++){
		var optionText = hiddenValueObj.options[i].text;
		if(optionText.indexOf(thisObjValue) == 0 && optionText.substring(0,optionText.indexOf(':')) == thisObjValue){
			var no = new Option();
			if((hiddenValueObj.options[i].value).indexOf(':')!=-1){
				no.value = (hiddenValueObj.options[i].value).substring((hiddenValueObj.options[i].value).indexOf(':')+1);
			}else{
				no.value = hiddenValueObj.options[i].value;
			}
			no.text = (hiddenValueObj.options[i].text).substring((hiddenValueObj.options[i].text).indexOf(':')+1);
			linkageSelectObj.options[linkageSelectObj.options.length] = no;
		}
	}
}
	//alert("linkageSelectObj.options.length22="+ linkageSelectObj.options.length);

}


function  getLinkageValue_CommonSelect(thisObj,linkageName){
	var thisObjText = thisObj.options[thisObj.selectedIndex].text;//得到一级的显示值
    var thisObjValue = thisObj.value;//得到一级的实际值
	var linkageObj = document.all(linkageName);//二级下拉框
	var linkageHIddenObj = linkageObj.parentElement.firstChild.nextSibling;//二级隐藏下拉框
	
	//清空联动下拉中的数据
	linkageObj.options.length = 0;
	var no = new Option();
	no.value = "";
	no.text = "请选择";
	linkageObj.options[linkageObj.options.length] = no;

	if(thisObjValue !=  "") {
		for(var i=0;i<linkageHIddenObj.options.length;i++){
			var optionText = linkageHIddenObj.options[i].text;
			if(optionText.indexOf(thisObjValue) == 0 && optionText.substring(optionText.indexOf(':')) == thisObjValue){
				var no = new Option();
				if((hiddenValueObj.options[i].value).indexOf(':')!=-1){
					no.value = (linkageHIddenObj.options[i].value).substring((linkageHIddenObj.options[i].value).indexOf(':')+1);
				}else{
					no.value = linkageHIddenObj.options[i].value;
				}
			
				no.text = (linkageHIddenObj.options[i].text).substring((linkageHIddenObj.options[i].text).indexOf(':')+1);
				linkageObj.options[linkageObj.options.length] = no;
			}
		}
	}
